 ...
 *
 * FIXES vs original:
 *  1. clip.filename was undefined — now generates clip_01.mp4, reel_01.mp4 etc.
 *  2. handleRemove used uploadInfo.filename — fixed to uploadInfo.publicId
 *  3. downloadAll used clip.filename — fixed
 *  4. Added "Save to Library" button per clip → POSTs to /api/resources/upload
 *  5. Added "Save All to Library" bulk button in results header
 */
...
const GREEN_BOR  = "rgba(22,163,74,0.25)";
...
export default function AdminVideoSplitter() {
  ...
  // savedIds: Set of clip publicIds already saved, so button turns green
  ...
  // ── Generate a display filename for a clip ────────────────────────────────
  const clipFilename = (clip, modeStr) => {
    if (modeStr === "ai") {
      const safe = (clip.title || `clip_${clip.rank}`)
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "_")
        .replace(/^_|_$/g, "")
        .slice(0, 40);
      return `rank${String(clip.rank).padStart(2, "0")}_${safe}.mp4`;
    }
    if (modeStr === "reels") return `reel_${String(clip.index).padStart(2, "0")}.mp4`;
    if (modeStr === "parts") return `part_${String(clip.index).padStart(2, "0")}.mp4`;
    return `clip_${String(clip.index).padStart(2, "0")}.mp4`;
  };

  // ─────────────────────────────────────────────────────────────────────────
  // SAVE TO LIBRARY  (posts the Cloudinary URL to /api/resources/upload)
  // ─────────────────────────────────────────────────────────────────────────
  const saveClipToLibrary = async (clip, modeStr) => {
    const clipId = clip.publicId || clip.index;
    if (savedIds.has(clipId) || savingIds.has(clipId)) return;

    setSavingIds((prev) => new Set([...prev, clipId]));

    try {
      const title    = modeStr === "ai"
        ? (clip.title || `AI Clip #${clip.rank}`)
        : `${modeStr === "reels" ? "Reel" : "Part"} ${clip.index}`;

      const formData = new FormData();
      formData.append("title",       title);
      formData.append("category",    "reels");          // saves under "reels" category
      formData.append("resourceUrl", clip.url);         // Cloudinary URL — no re-upload needed
      ...
      setSavedIds((prev) => new Set([...prev, clipId]));
      showSnack(`"${title}" saved to library ✓`);
    } catch {
      showSnack("Failed to save clip to library", "error");
    } finally {
      setSavingIds((prev) => {
  ...
  const saveAllToLibrary = async () => {
    ...
    setSavingAll(true);
    try {
      await Promise.all(clips.map((clip) => saveClipToLibrary(clip, results.mode)));
      showSnack(`All ${clips.length} clips saved to library ✓`);
    } catch {
      showSnack("Some clips failed to save", "error");
    } finally {
      setSavingAll(false);
    }
  };

  const saveSummaryToLibrary = async () => {
    ...
    try {
      ...
      showSnack("Failed to save summary to library", "error");
    } finally {
      setSavingAll(false);
    }
  };
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (mode === "ai") {
        res = await axios.post(
          `${API}/ai-clips`,
          { publicId: uploadInfo.publicId, url: uploadInfo.url, clipLength, contentType, clipCount },
          { headers: authHeaders(), timeout: 15 * 60 * 1000 }
        );
      } else if (mode === "reels") {
        res = await axios.post(
          `${API}/split/reels`,
          { publicId: uploadInfo.publicId, url: uploadInfo.url, clipDuration },
          { headers: authHeaders() }
        );
      } else if (mode === "parts") {
        res = await axios.post(
          `${API}/split/parts`,
          { publicId: uploadInfo.publicId, url: uploadInfo.url, parts: partsCount },
          { headers: authHeaders() }
        );
      } else {
        res = await axios.post(
          `${API}/split/summarize`,
          { publicId: uploadInfo.publicId, url: uploadInfo.url, targetDuration: summaryDuration },
          { headers: authHeaders() }
        );
      }
  ...
  // ── FIX: was using uploadInfo.filename — correct field is uploadInfo.publicId ──
  const handleRemove = async () => {
    if (uploadInfo) {
      await axios
        .delete(`${API}/upload/${encodeURIComponent(uploadInfo.publicId)}`, { headers: authHeaders() })
        .catch(() => {});
    }
    setVideoFile(null);
    setUploadInfo(null);
  ...
  // ── FIX: was using clip.filename (undefined) — now uses generated name ──
  const downloadAll = () => {
    const clips = results?.clips || [];
    clips.forEach((clip, i) => {
      setTimeout(() => {
        const a    = document.createElement("a");
        a.href     = clip.url;
        a.download = clipFilename(clip, results.mode);
        a.target   = "_blank";
        a.click();
      }, i * 400);
    });
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // SAVE BUTTON component (reused for each clip)
  // ─────────────────────────────────────────────────────────────────────────────
  const SaveBtn = ({ clip, modeStr, small = false }) => {
    const clipId  = clip.publicId || clip.index;
    const saved   = savedIds.has(clipId);
    const saving  = savingIds.has(clipId);

    return (
      <Tooltip title={saved ? "Saved to Library" : "Save to Library"} placement="top">
        <IconButton
          size="small"
          onClick={() => saveClipToLibrary(clip, modeStr)}
          disabled={saved || saving}
          sx={{
            borderRadius: "8px",
            p: small ? 0.5 : 0.6,
            color: saved ? GREEN : TEXT_MUTED,
            background: saved ? GREEN_BG : "transparent",
            border: `1px solid ${saved ? GREEN_BOR : "transparent"}`,
            "&:hover": {
              color:      saved ? GREEN : NAV_DARK,
              background: saved ? GREEN_BG : BG_CHIP,
            },
            "&:disabled": {
              color:      saved ? GREEN : "#ccc",
              background: saved ? GREEN_BG : "transparent",
            },
          }}
        >
          {saved
            ? <CheckCircleIcon sx={{ fontSize: small ? 15 : 16 }} />
            : saving
            ? <CloudUploadIcon sx={{ fontSize: small ? 15 : 16 }} />
            : <SaveIcon sx={{ fontSize: small ? 15 : 16 }} />}
        </IconButton>
      </Tooltip>
    );
  };
    ...
    <>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: { xs: 2, sm: 3 }, alignItems: "start" }}>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
              ...
              <Box sx={{ background: "#fff", border: `1px solid ${BORDER_COL}`, borderRadius: "16px",
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                  {MODES.map((m) => (
                    <Box key={m.id} onClick={() => { setMode(m.id); setResults(null); setError(null); setSavedIds(new Set()); }} sx={{
                      display: "flex", alignItems: "center", gap: 1.5,
                      p: "10px 14px", borderRadius: "12px", cursor: "pointer",
                      border: `1.5px solid ${mode === m.id ? (m.id === "ai" ? PURPLE : NAV_DARK) : BORDER_COL}`,
                      background: mode === m.id ? (m.id === "ai" ? PURPLE : NAV_DARK) : "#fff",
              ...
              {results?.mode === "ai" && results.clips && (
                <Box sx={{ background: "#fff", border: `1px solid ${PURPLE_BOR}`, borderRadius: "16px",
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2, gap: 1, flexWrap: "wrap" }}>
                    ...
                    {/* ── Bulk action buttons ── */}
                    <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                      <Button size="small" startIcon={<SaveIcon sx={{ fontSize: 13 }} />}
                        onClick={saveAllToLibrary} disabled={savingAll}
                        sx={{
                          fontFamily: FONT_BODY, fontWeight: 700, fontSize: 12, textTransform: "none",
                          color: GREEN, borderRadius: "8px", border: `1px solid ${GREEN_BOR}`,
                          px: 1.5, py: 0.5,
                          "&:hover": { background: GREEN_BG },
                          "&:disabled": { color: "#aaa", borderColor: "#e8e8e8" },
                        }}>
                      ...
                      <Button size="small" startIcon={<DownloadIcon sx={{ fontSize: 14 }} />}
                        ...
                        sx={{
                          fontFamily: FONT_BODY, fontWeight: 700, fontSize: 12, textTransform: "none",
                          color: PURPLE, borderRadius: "8px", border: `1px solid ${PURPLE_BOR}`,
                          px: 1.5, py: 0.5,
                          "&:hover": { background: PURPLE_BG },
                        }}>
                        Download All
                    ...
                    {results.clips.map((clip) => (
                      <Box key={clip.index} sx={{
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between",
                          ...
                          {/* ── Per-clip action icons ── */}
                          <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                            <SaveBtn clip={clip} modeStr="ai" />
                            <Tooltip title="Download" placement="top">
                              <IconButton component="a" href={clip.url}
                                download={clipFilename(clip, "ai")} target="_blank" size="small"
                                sx={{ color: PURPLE, borderRadius: "8px", p: 0.6,
                                  "&:hover": { background: "rgba(124,58,237,0.15)" } }}>
                                <DownloadIcon sx={{ fontSize: 16 }} />
                              </IconButton>
              ...
              {results?.mode === "summarize" && results.summary && (
                <Box sx={{ background: "#fff", border: `1px solid ${BORDER_COL}`, borderRadius: "16px",
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between",
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                      <Tooltip title="Save to Library" placement="top">
                        <IconButton onClick={saveSummaryToLibrary} disabled={savingAll} size="small"
                          sx={{ color: TEXT_MUTED, borderRadius: "8px", p: 0.8,
                            "&:hover": { color: GREEN, background: GREEN_BG },
                            "&:disabled": { color: "#ccc" } }}>
                          <SaveIcon sx={{ fontSize: 18 }} />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="Download" placement="top">
                        <IconButton component="a" href={results.summary.url}
                          download="summary.mp4" target="_blank" size="small"
                          sx={{ color: TEXT_MUTED, borderRadius: "8px", p: 0.8,
                            "&:hover": { color: NAV_DARK, background: BG_CHIP } }}>
                          <DownloadIcon sx={{ fontSize: 18 }} />
                        </IconButton>
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2, gap: 1, flexWrap: "wrap" }}>
                    ...
                    <Box sx={{ display: "flex", gap: 1 }}>
                      ...
                      <Button size="small" startIcon={<DownloadIcon sx={{ fontSize: 14 }} />}
                        ...
                        sx={{
                          fontFamily: FONT_BODY, fontWeight: 700, fontSize: 12, textTransform: "none",
                          color: NAV_DARK, borderRadius: "8px", border: "1px solid #e8e8e8",
                          px: 1.5, py: 0.5,
                          "&:hover": { background: BG_CHIP },
                        }}>
                        Download All
                    ...
                    {results.clips.map((clip) => (
                      <Box key={clip.index} sx={{ display: "flex", alignItems: "center",
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                          ...
                          <Box>
                            <Typography sx={{ fontFamily: FONT_BODY, fontWeight: 600, fontSize: 13, color: NAV_DARK }}>
                              {clipFilename(clip, results.mode)}
                            </Typography>
                            <Typography sx={{ fontFamily: FONT_BODY, fontSize: 11, color: TEXT_MUTED }}>
                              Start {clip.startTime} · {clip.duration}
                            </Typography>
                        ...
                        {/* ── Per-clip action icons ── */}
                        <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                          <SaveBtn clip={clip} modeStr={results.mode} small />
                          <Tooltip title="Download" placement="top">
                            <IconButton component="a" href={clip.url}
                              download={clipFilename(clip, results.mode)} target="_blank" size="small"
                              sx={{ color: TEXT_MUTED, borderRadius: "8px", p: 0.6,
                                "&:hover": { color: NAV_DARK, background: "#e8e8e8" } }}>
                              <DownloadIcon sx={{ fontSize: 16 }} />
                            </IconButton>
      ...
      >
        <Alert
          ...
          sx={{ fontFamily: FONT_BODY, fontWeight: 600, borderRadius: "12px" }}
        >
          {snackbar.message}
        </Alert>
      </Snackbar>
