...
const Dashboard = () => {
  ...
  // ── FIX: Use sessionStorage to persist admin status across route changes ──
  // When landing on /admin-dashboard, stamp it in sessionStorage.
  // On every other route we read from sessionStorage so the flag survives
  // navigating into course sub-routes like /dsac, /cou, etc.
  const isAdminSession = sessionStorage.getItem("isAdminSession") === "true";
  if (location.pathname === "/admin-dashboard") {
    sessionStorage.setItem("isAdminSession", "true");
  }
  const isAdminRoute = location.pathname === "/admin-dashboard" || isAdminSession;
  ...
  // ── FIX: reads isAdminRoute which now correctly stays true even on /dsac etc. ──
  const goBackToDashboard = () => {
    if (isAdminRoute) {
      navigate("/admin-dashboard");
      setActiveView(VIEW_ADMIN);
  ...
  /* ─────────── LOGOUT ─────────── */
  const handleLogout = () => {
    // ── FIX: clear admin session flag on logout ──
