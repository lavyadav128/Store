...
function renderMarkdown(text) {
  ...
  html = html.replace(
    /^(\d+)\. (.+)$/gm,
    "<li class='ol-item'><span class='ol-num'>$1.</span> $2</li>"
  );
...
const S = {
  ...
  inputRow: (isMobile) => ({
    padding: isMobile
      ? "10px 12px calc(env(safe-area-inset-bottom) + 12px)"
      : "8px 14px 12px",
...
const GLOBAL_CSS = `
  *{
  ...
  .copilot-md ul li.ul-item{
    padding:2px 0 2px 16px;
    position:relative;
    line-height:1.6;
  }

  .copilot-md ul li.ul-item::before{
    content:"•";
    position:absolute;
    left:4px;
    color:#1D9E75;
  ...
  .copilot-md ol li.ol-item{
    padding:2px 0 2px 28px;
    position:relative;
    line-height:1.6;
  }

  .copilot-md ol li.ol-item .ol-num{
    position:absolute;
    left:4px;
    color:#1D9E75;
    font-weight:600;
...
function enrichMessage(message, topicSlug) {
  ...
  if (/summar/i.test(lower))
    return `Summarise "${topic}" — cover what it is, why it matters, and the main concepts.`;

  if (/key point/i.test(lower))
    return `List the key points of "${topic}" in bullet form.`;

  if (/quiz/i.test(lower))
    return `Give me a 3-question quiz about "${topic}" with answers.`;

  if (/explain/i.test(lower))
    return `Explain "${topic}" with a simple example.`;
...
// BOT BUBBLE
// ─────────────────────────────────────────────
function BotBubble({ text, isMobile }) {
  return (
    <div
      style={S.bubble("bot", isMobile)}
      className="copilot-md"
...
export default function ChatbotWidget() {
  ...
  const handlePdfSelect = useCallback(async (e) => {
    ...
    const base64 = await new Promise((res, rej) => {
      ...
      r.onerror = () =>
        rej(new Error("File read failed"));
  ...
  return (
    <>
      ...
      {/* FAB */}
      <div style={S.fab(isMobile)}>
        <button
          style={S.fabBtn(open, isMobile)}
          onClick={() => setOpen((v) => !v)}
          aria-label={
            open
              ? "Close study copilot"
      ...
      {/* PANEL */}
      <div
        style={S.panel(open, isMobile)}
        role="dialog"
        aria-label="AI Study Copilot"
        aria-modal="true"
      >
        {/* HEADER */}
        <div style={S.header(isMobile)}>
          <div style={S.headerIcon(isMobile)}>
            🌿
          </div>

          <div>
            <p style={S.headerTitle(isMobile)}>
              Study Copilot
            </p>

            <p style={S.headerSub(isMobile)}>
              Powered by your course material
            </p>
          </div>

          <div style={S.headerActions}>
            <button
              style={S.iconBtn(isMobile)}
              title="Upload PDF"
              onClick={() =>
        ...
        {/* CONTEXT */}
        <div style={S.contextPill(isMobile)}>
          <span>📍</span>

          <span>
            {topicLabel || "detecting topic…"}
          </span>
        </div>
        ...
        {/* ERROR */}
        {apiError && (
          <div style={S.errorBanner(isMobile)}>
            ⚠️ {apiError}
          </div>
        )}
        ...
        <div style={S.messagesArea(isMobile)}>
          ...
          {loading && (
            ...
            >
              <div style={S.typingWrap(isMobile)}>
                <span style={dot(0)} />
                <span style={dot(0.2)} />
                <span style={dot(0.4)} />
              </div>
        ...
        {/* SUGGESTIONS */}
        {showSuggestions && !loading && (
          <div style={S.suggestionsRow(isMobile)}>
            {DEFAULT_SUGGESTIONS.map((s) => (
              <button
                key={s}
                style={S.chip(isMobile)}
                onClick={() =>
                  applySuggestion(s)
                }
              >
