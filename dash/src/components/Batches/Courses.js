...
// // ─────────────────────────────────────────────────────────────
// // STATIC DATA — the list of all available classes
// // ─────────────────────────────────────────────────────────────

// // classList is a plain JavaScript array — it never changes, so it lives outside the component
// // Each object represents one class card that will be shown on the page
// // id       = unique identifier for this class (used in URLs and purchase records)
// // title    = display name shown on the card
// // description = short text shown on the card
// // imageUrl = path to the card's thumbnail image (served from /public/images/)
// // price    = cost in rupees; 0 means free
// const classList = [
//   {
//     id: '10',
//     title: 'Class 10',
...
import {
  ...
  Button, Dialog, DialogContent, IconButton, useTheme, useMediaQuery,
} from "@mui/material";
...
const CombinedClassPage = () => {
  ...
  // ── ONLY NEW STATE: batches now from API instead of hardcoded ──
  ...
  // ── FETCH PURCHASES — identical to your original ──
  useEffect(() => {
    const fetchPurchases = async () => {
      try {
        const data = await makeAuthenticatedRequest(`${server}/api/user-purchases`);
        const info = {};
        // batches may not be loaded yet so we check all purchases generically
  ...
  // ── PURCHASE HANDLER — identical to your original ──
  const handleBuyRedirect = async (batch) => {
    const token = localStorage.getItem("token");
    if (!token) { alert("Please login first"); return; }

    const purchasePayload = {
      classId: batch.id, batchTitle: batch.title, price: batch.price,
  ...
  return (
    <Box sx={{ flexGrow: 1, py: isMobile ? 2 : 0, px: isMobile ? 1.5 : 5 }}>
      <Typography variant={isMobile ? "h5" : "h4"} fontWeight={700}
        mb={4} textAlign="center" color="primary">
        DSA to Deployment
      </Typography>
      ...
      <Box sx={{
        ...
        {batches.map((batch) => {
          ...
          const expiryDate = purchaseInfo[batch.id]?.expiryDate
            ? new Date(purchaseInfo[batch.id].expiryDate) : null;
          ...
          return (
            <Box key={batch.id} sx={{
              ...
              <Card sx={{
                ...
                <CardContent sx={{ px: 3, pt: 0 }}>
                  ...
                  <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mt: 2 }}>
                    <Button variant="contained" color={batch.price === 0 ? "success" : "warning"}
                      sx={{ borderRadius: "50px", pointerEvents: "none", px: 2, py: 0.5,
                        fontWeight: 700, fontSize: "0.85rem", minWidth: "80px", textTransform: "none" }}>
                      {batch.price === 0 ? "FREE" : `₹${batch.price}`}
                    </Button>
                    {isPurchased && (
                      <Box sx={{ bgcolor: "#1976d2", color: "#fff", fontWeight: 500,
                        px: 1.5, py: 0.3, borderRadius: 50, fontSize: "0.85rem" }}>
                        Purchased
                      </Box>
                    )}
                  </Box>
                ...
                <CardActions sx={{ px: 3, pb: 3, pt: 0, justifyContent: "space-between", mt: "auto" }}>
                  ...
                  <Button variant="contained" color="primary"
                    onClick={() => isPurchased ? navigate(batch.redirectPath) : handleBuyRedirect(batch)}
                    sx={{ width: "48%", fontWeight: 600, borderRadius: 2, textTransform: "none" }}>
                    {isPurchased ? "Study" : "Buy Now"}
                  </Button>
                </CardActions>
              ...
              {/* Screenshot Modal — identical to original */}
              <Dialog open={openModalId === batch.id} onClose={() => setOpenModalId(null)}
                ...
                <DialogContent sx={{ position: "relative", p: 3, textAlign: "center" }}>
                  ...
                  <Box sx={{ display: "flex", justifyContent: "center" }}>
                    <img src={batch.screenshot} alt={`${batch.title} Preview`}
                      style={{ maxWidth: "100%", maxHeight: "400px", borderRadius: 10 }} />
                  </Box>
                </DialogContent>
              </Dialog>

              {/* Explore Modal — identical to original */}
              <Dialog open={openExploreId === batch.id} onClose={() => setOpenExploreId(null)}
                maxWidth="sm" fullWidth>
                <DialogContent sx={{ position: "relative", p: 3 }}>
                  <IconButton onClick={() => setOpenExploreId(null)}
                    sx={{ position: "absolute", top: 8, right: 8 }}>
                    <CloseIcon />
                  </IconButton>
                  ...
                  <Box component="ul" sx={{ pl: 2, m: 0 }}>
                    {batch.whatYouLearn.map((point, index) => (
                      <Typography key={index} component="li" variant="body1"
                        ...
                        sx={{ mb: 1.4, fontSize: "1rem", listStyle: "none", pl: 3,
                          position: "relative",
                  ...
                  <Button variant="outlined" color="secondary"
                    ...
                    sx={{ mt: 3, fontWeight: 600, borderRadius: 2, display: "block",
                      mx: "auto", textTransform: "none" }}>
                    View Screenshot
                  </Button>
                </DialogContent>
              </Dialog>
