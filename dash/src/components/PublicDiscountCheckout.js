...
const server = process.env.REACT_APP_BACKEND_URL || 'https://store-c39h.onrender.com';
...
const PublicDiscountCheckout = () => {
  ...
  useEffect(() => {
    const fetchOffer = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`${server}/api/recovery-offers/public/${offerId}`);
        if (res.data.success) {
          setOfferData(res.data);
          setEmail(res.data.customerEmail || '');
          setName(res.data.customerName || 'Learner');
        } else {
