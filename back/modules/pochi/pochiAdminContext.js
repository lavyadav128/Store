...
import AgentAction from '../revenue-recovery/schema/AgentAction.model.js';
import AgentPolicy from '../revenue-recovery/schema/AgentPolicy.model.js';
import InstagramContent from '../instagram-agent/schema/InstagramContent.model.js';
import InstagramAgentConfig from '../instagram-agent/schema/InstagramAgentConfig.model.js';
import ClientProject from '../client-agent/schema/ClientProject.model.js';
import ClientAgentConfig from '../client-agent/schema/ClientAgentConfig.model.js';
import Resource from '../notes/schema/Resource.model.js';
...
import { generateContentDraft, publishDueContent, startInstagramAgent, stopInstagramAgent } from '../instagram-agent/services/instagramAgent.service.js';
import { updatePolicy } from '../revenue-recovery/services/policyService.js';
...
export async function askPochi(query = '', adminUser, history = []) {
  ...
  // Instagram Growth Agent Actions: Draft quote / start / stop / publish
  if (/(?:draft|create|generate)\s+(?:a\s+)?(?:new\s+)?(?:quote|post|reel)(?:\s+on|\s+about)?\s*(.*)/i.test(q)) {
    const topicMatch = q.match(/(?:draft|create|generate)\s+(?:a\s+)?(?:new\s+)?(?:quote|post|reel)(?:\s+on|\s+about)?\s*(.*)/i);
    const categoryOrTopic = topicMatch?.[1]?.trim() || "Life Wisdom";
    try {
      const draft = await generateContentDraft({ topic: categoryOrTopic, category: categoryOrTopic });
      return {
        voiceText: `I have drafted a unique ${draft.themeCategory || 'motivational'} quote by ${draft.speaker || 'a master speaker'} with the Ultimate Dreams anthem attached, Admin.`,
        visualReply: `Drafted: **"${draft.quote?.slice(0, 80)}..."** — ${draft.speaker} (${draft.themeCategory}) with 🎵 **Ultimate Dreams Anthem**.`,
        action: "NAVIGATE_VIEW",
        targetView: "instagram",
      };
    } catch (e) {
      console.warn("Pochi draft error:", e.message);
    }
  }

  if (/(?:start|turn on|activate|enable)\s+(?:the\s+)?instagram(?:\s+agent|\s+growth\s+agent)?/i.test(q)) {
    await startInstagramAgent();
    return {
      voiceText: "Instagram Growth Agent is now running and scheduled for autonomous daily posting.",
      visualReply: "Started **Instagram Growth Agent** (Autonomous daily publishing enabled).",
      action: "NAVIGATE_VIEW",
      targetView: "instagram",
    };
  }

  if (/(?:stop|pause|turn off|disable)\s+(?:the\s+)?instagram(?:\s+agent|\s+growth\s+agent)?/i.test(q)) {
    await stopInstagramAgent();
    return {
      voiceText: "I have paused the Instagram Growth Agent.",
      visualReply: "Paused **Instagram Growth Agent**.",
      action: "NAVIGATE_VIEW",
      targetView: "instagram",
    };
  }

  if (/(?:publish|post)\s+(?:now|due|scheduled|latest)(?:\s+post|\s+quote|\s+reel)?/i.test(q)) {
    const pubRes = await publishDueContent();
    return {
      voiceText: pubRes.published > 0
        ? `Successfully published ${pubRes.published} due post to Instagram with the Ultimate Dreams anthem, Admin.`
        : "There are no scheduled posts due for publishing right now.",
      visualReply: `Published **${pubRes.published}** posts to Instagram.`,
      action: "NAVIGATE_VIEW",
      targetView: "instagram",
    };
  }
  ...
  // 3D Visualization & Interactive Simulation Intent
  if (/(show|view|display|render|see|open|rotate|browse).*(3d|three\s*d|3-d|animation|simulation|model)/i.test(q) || /(3d|three\s*d|3-d)\s+(view|model|animation|image|heart|car|planet|atom|robot|dna|earth|galaxy|tree|building)/i.test(q)) {
    const cleanTopic = q.replace(/(show me|show|display|render|in|animation|3d|three d|3-d|view|image|of|a|an|the|model|please|can you|pochi)/gi, '').trim() || 'Interactive 3D Simulation';
    const formattedTitle = cleanTopic.charAt(0).toUpperCase() + cleanTopic.slice(1);
    return {
      voiceText: `Rendering an interactive 3D model of ${formattedTitle}. You can rotate it 360 degrees, zoom, and explore it in 3D right on your screen.`,
      visualReply: `Rendering interactive 3D simulation for **${formattedTitle}**. Drag to rotate 360°, scroll to zoom, and toggle wireframe or lighting modes.`,
      action: "SHOW_3D_MODEL",
      model3d: {
        title: formattedTitle,
        query: cleanTopic.toLowerCase(),
        type: cleanTopic.toLowerCase(),
        animation: true,
        interactive: true,
        description: "Interactive 3D simulation with 360° orbit rotation, zoom, and real-time lighting.",
      },
    };
  }
...
1. Interactive 3D Visualizations: When the admin asks to see/show anything in 3D (e.g. "show 3D heart", "3D car", "3D view of planet"), set action to "SHOW_3D_MODEL" and include "model3d": { "title": "Topic", "query": "topic" }.
2. Immediate Retries & Manual Admin Approvals: Run as soon as triggered or approved by the Admin in the dashboard.
...
- Return a JSON object:
{
  ...
  "action": "NONE" | "NAVIGATE_VIEW" | "SHOW_3D_MODEL",
  "targetView": null | "revenue" | "instagram" | "client_agent" | "dreams" | "batches",
  "model3d": null | { "title": "string", "query": "string" }
}`;
